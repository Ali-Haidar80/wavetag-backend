...
import { AuthenticatedRequest, DecodedToken } from "../interfaces/auth.interfaces";
...
const logoutUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const decoded: DecodedToken = jwt.verify(token, process.env.JWT_SECRET_KEY!) as DecodedToken;
    ...
    res.status(200).json({ message: 'User logged out successfully', expiredToken });
  } catch (err) {
    res.status(401);
 ...
 */
const refresh = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (newtoken.exp !== undefined && newtoken.exp <= Date.now() / 1000) {    
    ...
    res.status(400);
    throw new Error('Token is not expired yet');
  }
